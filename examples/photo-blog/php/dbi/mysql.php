...
class MySQL {
		//hold the last query ran
	private static $last_query = '';
	private static $db = null;    //hold current connection.
	 ...
	 * @param	boolean	$replace  Set to true to do a REPLACE instead of INSERT
	 * @param	boolean	$add_ignore  Set to true to do a INSERT IGNORE instead of just INSERT
	 * @return  boolean
	 */
	public static function insert($table, $items, $replace=false, $add_ignore=false) {
		
		if(empty($table) || !is_array($items) || !count($items)) {
			return false;
		}
		$field_names = array();
		...
		if($replace) {
			...
			//build rest of query
		$sql .= " INTO `{$table}` ({$field_name_sql}) VALUES({$field_placeholder_sql})";
	 ...
	 * @return  boolean
	 */
	public static function replace($table, $items) {
			//this is just a special kind of insert
		return self::insert($table, $items, true);
	}
	 ...
	 * @return  boolean
	 */
	public static function insert_ignore($table, $items) {
			//this is just a special kind of insert
		return self::insert($table, $items, false, true);
	}
	 ...
	 * @return	boolean
	 */
	public static function update($table, $items, $conditions=null) {
		
		if(empty($table) || !count($items)) {
			return false;
		}
		...
		$sql = "UPDATE {$table} SET {$update_sql} {$where_sql}";
	 ...
	 * @return	boolean
	 */
	public static function increment($table, $field, $conditions=null, $amount=1) {
	 ...
	 * @return	boolean
	 */
	public static function remove($table, $conditions=null) {
	 ...
	 * @return	mixed
	 */
	public static function value($table, $get_field, $conditions=null, $order=null) {
			//dont allow empty where conditions
	 ...
	 * @return  integer
	 */
	public static function total($table, $conditions=null) {
	 ...
	 * @param	integer	$start	Row offset (Default: 0)
	 * @param	integer	$count	Row count to return (Default: 50)
	 * @param	array	$order	An array of values to set the order by
	 * @param	boolean	$numeric_index	Return rows with an indexed
	 * @return	array
	 */
	public static function rows($table, $conditions=null, $start=0, $count=50, $order=null, $numeric_index=false) {
	...
	public static function query($sql, $data=array(), $check_errors=true) {
		...
		if(!self::$db && !self::connect()) {
			\Core\Application::log_msg('Could not (re)connect to database', 1, __FILE__, __LINE__);
			return false;
		}
